...
**Единая функция `resolveFreightRate(cargo, vessel, distanceNm) → { value, source, confidence }`**
— водопад по приоритету:
...
- `resolveFreightRate()` — новая функция-водопад, ЕДИНАЯ точка получения ставки.
