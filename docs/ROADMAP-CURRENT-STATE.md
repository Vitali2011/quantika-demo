...
**Текущая версия:** prod HEAD `8d4b7d3c` (demo-polish кампания #745–750 + прод-сид-патч speed/consumption+market-history; main `c051f219` +dependabot #752) auto-deploy LIVE на outreach-vps
**Статус:** 🟢 Production-quality SaaS UI + backend wiring ~90%. **Demo-polish кампания ЗАКРЫТА по работе** (все фичи в проде + сид пропатчен) — ждёт founder прод-приёмку (5 экранов: /cargo QTY, /match laycan+P&L+bunker+EUA, /market 30-дн, дашборд-дата). Открытые followup (не из этой кампании): (1) rate-limiter `/api/parser/email` (MEDIUM, non-blocking), (2) Mode M1 = UX-choice (принято), (3) R6.5 dark-mode toggle, (4) components/ui 40 stale imports.
