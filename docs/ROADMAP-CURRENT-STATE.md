...
**Текущая версия:** prod HEAD `f0e26e85` (demo-polish #745–750 + пост-приёмка #760 match-laycan + #761 drewry; прод-сид в `demo-seed.db`: speed/consumption + market 30-дн + drewry) auto-deploy LIVE на outreach-vps
**Статус:** 🟢 Production-quality SaaS UI + backend wiring ~90%. **Demo-polish кампания ЗАКРЫТА + приёмка ПРОЙДЕНА 2026-06-02** (founder верифицировал все 5 экранов: /cargo QTY ✅, /match laycan+P&L+bunker+EUA ✅, /market 30-дн+drewry ✅, дашборд-дата ✅). Открытые followup (не из этой кампании): (1) rate-limiter `/api/parser/email` (MEDIUM, non-blocking), (2) Mode M1 = UX-choice (принято), (3) R6.5 dark-mode toggle, (4) components/ui 40 stale imports.
