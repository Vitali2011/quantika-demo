...
**Статус:** 🟢 Production-quality SaaS UI + backend wiring ~90%. **Matching-gates+re-parse кампания на проде 2026-06-02; 4-fix WIP (`feat/matching-4fixes-wip`) — доделать (#2 EU-детект vague-порты, #3 регенный laycan-путь).** Demo-polish кампания ЗАКРЫТА + приёмка ПРОЙДЕНА 2026-06-02 (founder верифицировал все 5 экранов: /cargo QTY ✅, /match laycan+P&L+bunker+EUA ✅, /market 30-дн+drewry ✅, дашборд-дата ✅). Открытые followup (не из этой кампании): (1) rate-limiter `/api/parser/email` (MEDIUM, non-blocking), (2) Mode M1 = UX-choice (принято), (3) R6.5 dark-mode toggle, (4) components/ui 40 stale imports.
