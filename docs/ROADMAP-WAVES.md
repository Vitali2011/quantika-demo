...
## Wave γ — Vertex AI / Gemini migration (12 спек + GPT-5.5 rollback)

**Добавлено:** 2026-05-04. Полный план: [`docs/waves/wave-gamma-vertex-migration.md`](waves/wave-gamma-vertex-migration.md).

**Бюджет:** GenAI App Builder credit ($900 / zł3,713, expires 15 апреля 2027) на GCP проект `quantika-demo-2026`.

**Хард-требование:** rollback на gpt-5.5 одним env var в любой момент. Архитектура — `lib/ai-provider.ts` shim + per-endpoint `*_PROVIDER` env, готовый пресет `.env.gpt-fallback.example`.

**Спеки (порядок merge'а от меньшего риска к большему):**

| ID        | Описание                                                           | Effort | Rollback flag                             |
| --------- | ------------------------------------------------------------------ | ------ | ----------------------------------------- |
| **γv-00** | `ai-provider-shim` (foundation, обязательно первым)                | 0.5d   | —                                         |
| γv-07     | recap: убрать slice(0, 2000), Gemini Pro 1M context                | 0.25d  | `RECAP_PROVIDER=openai`                   |
| γv-05     | draft-quote / draft-reply → Flash                                  | 0.25d  | `DRAFT_QUOTE_PROVIDER=openai`             |
| γv-03     | parse-vessel → Flash                                               | 0.25d  | `PARSE_VESSEL_PROVIDER=openai`            |
| γv-02     | parse-cargo → Flash                                                | 0.5d   | `PARSE_CARGO_PROVIDER=openai`             |
| γv-01     | classify → Flash                                                   | 0.5d   | `CLASSIFY_PROVIDER=openai`                |
| γv-04     | parse-recap → Pro                                                  | 0.5d   | `PARSE_RECAP_PROVIDER=openai`             |
| γv-09     | image OCR → Gemini Vision (multi-image в одном вызове)             | 1d     | `WHATSAPP_OCR_PROVIDER=openai`            |
| γv-08     | WhatsApp voice → Gemini 2.0 Flash audio (Arabic+EN code-switching) | 0.5d   | `WHATSAPP_VOICE_PROVIDER=openai`          |
| γv-10     | LLM agent planner (заменить regex `detectKinds`)                   | 1d     | `AGENT_PLANNER_PROVIDER=regex`            |
| γv-06     | match → Pro (последним, наиболее сложный prompt)                   | 1d     | `MATCH_PROVIDER=openai`                   |
| γv-11     | "Explain this deal" wow-фича для Дубая                             | 1.5d   | feature flag `EXPLAIN_DEAL_ENABLED=false` |
| γv-12     | Imagen 4 — карта маршрута для demo                                 | 1d     | feature flag `ROUTE_MAP_ENABLED=false`    |

**Total effort:** ~9 человеко-дней без stretch'ей.

**Stretch (после wave-γ, если бюджет токенов позволит):** match с историей сделок (1d), Semantic sanctions через embeddings (1.5d), Vertex AI Search над архивом сделок (4d).

**Защита от перерасхода:** Budget alert "Quantika Demo budget alert" в Cloud Console — zł100/мес после вычета кредитов, триггеры 50/90/100% на email vitali6825621@gmail.com. Email на 50% = красный флаг (кредиты кончились), немедленный rollback через `cp .env.gpt-fallback.example .env.local`.
...
_Документ сгенерирован: 2026-04-28_
_Источники: QUANTIKA-UX-VISION.md · quantika-architecture-audit-2026-04-24.md · quantika-architecture-plain-2026-04-24.md_
_Wave pipeline: `/Users/jarvis/claude/skills/wave-pipeline/SKILL.md`_
