...
echo "2. Run: PREFLIGHT_MODE=prod bash scripts/preflight.sh  # fail-fast on placeholders"
echo "3. Run: npm install --include=dev && npm run build"
echo "4. Run: pm2 start ecosystem.config.js && pm2 save"
echo "5. Add ops/Caddyfile.demo.quantika.org to /etc/caddy/Caddyfile"
echo "6. Run: caddy reload --config /etc/caddy/Caddyfile"
echo ""
echo "For redeploys: bash scripts/redeploy.sh  # pulls, preflights, rebuilds, restarts"
