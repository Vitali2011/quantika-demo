/**
 * Bosporus Strait fee calculator (flat fee by vessel type, no SCNT).
 *
 * Input Contract:
...
export function quoteBosporus(input: CanalInput): CanalQuote {
  ...
  const tariff = queryTariff('bosporus', vesselType, 0);
  if (!tariff) {
    throw new Error(`No active Bosporus tariff found for vessel_type=${vesselType}`);
  }
